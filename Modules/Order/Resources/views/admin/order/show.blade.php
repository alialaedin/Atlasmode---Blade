  ...
  <x-slot name="cardBody">
    @php
      ...
      $customerDetail = [
        ...
        ['title' => 'تاریخ تولد', 'value' => $order->customer->birth_date ? verta($order->customer->birth_date)->format('Y/m/d') : null],
        ['title' => 'جنسیت', 'value' => $genders[$order->customer->gender]],
...
@section('scripts')
  <script>
    ...
    function updateOrderItemStatus() {
      ...
      let getNewItemStatus = (orderItem) => {
        return orderItem.status ? 0 : 1;
      };
      ...
      $('.edit-item-status-button').each(function () {
        ...
        $(this).click(() => {
          ...
          let newStatus = getNewItemStatus(orderItem);
